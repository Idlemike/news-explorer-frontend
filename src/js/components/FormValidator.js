...
// Валидация
  /** Функция проверки поля на ошибки, возвращает истину если
   * поле валидно или ложь в противном случае, устанавливает
   * кастомное сообщение об ошибке */
  isValidate = (input) => {
    input.setCustomValidity(''); // устанавливаем свойсво validity.customError в false
    // если на инпуте есть атрибут required, поле validity.valueMissing будет true
    // / false (заполнено)
    ...
    // если на инпуте есть атрибут minlength, поле validity.tooShort
    // будет true / false (достигнута мин. длина)
    if (input.validity.tooShort || input.validity.tooLong) {
      input.setCustomValidity(errorMessages.wrongLength);
      return false;
    }
    // если на инпуте есть атрибут type, поле validity.typeMismatch
    // будет true / false (сопадение типа)
    ...
    /** Функция добавления/удаления ошибки с инпута, возвращает
     * true если поле валидно, false в противном случае */
    isFieldValid = (input) => {
    ...
    handlerInputForm = (event) => {
      ...
      const [...inputs] = event.currentTarget.elements; // превращаем
      // итератор(итерируемый объект) в массив
      this.isFieldValid(event.target); // проверяем поле на валидность и
      // выводим ошибку если не валидно.
      if (inputs.every(this.isValidate)) { // если каждый инпут формы
        // вернул true, то включаем кнопку в противном случае выключаем
